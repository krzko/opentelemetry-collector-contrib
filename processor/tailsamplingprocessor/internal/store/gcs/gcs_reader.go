...
// GCSReader implements SegmentReader for Google Cloud Storage.
// It follows the Single Responsibility Principle by focusing solely on
// reading and decoding segments from GCS.
...
// NewGCSReader creates a new GCS segment reader
func NewGCSReader(client GCSClient) (*GCSReader, error) {
	if client == nil {
...
func (r *GCSReader) SetDecoder(decoder Decoder) {
	r.decoder = decoder
}

// SetAvroReader sets the Avro reader (dependency injection)
func (r *GCSReader) SetAvroReader(reader AvroReader) {
	r.avroReader = reader
}

// OpenSegment prepares the reader for streaming spans from the given segment
func (r *GCSReader) OpenSegment(ctx context.Context, ref spill.SegmentRef) error {
	if r.closed {
	...
	// Parse the GCS URL
	bucket, object, err := r.parseGCSURL(ref.URL)
	if err != nil {
	...
	if objectHandle == nil {
		return fmt.Errorf("failed to get object: %s", object)
	}

	// Create reader for the object
	reader := objectHandle.NewReader(ctx)
	if reader == nil {
		return fmt.Errorf("failed to create reader for object: %s", object)
	}
	...
	switch ref.Codec {
	case spill.Avro:
		if err := r.setupAvroReader(reader); err != nil {
			reader.Close()
			return fmt.Errorf("failed to setup Avro reader: %w", err)
		}

	case spill.JSONLZstd:
		if err := r.setupJSONLReader(reader); err != nil {
			reader.Close()
...
func (r *GCSReader) setupAvroReader(reader io.Reader) error {
	if r.avroReader == nil {
...
func (r *GCSReader) setupJSONLReader(reader io.Reader) error {
	if r.decoder == nil {
...
func (r *GCSReader) Next(ctx context.Context) (spill.SpanRecord, error) {
	if r.closed {
...
func (r *GCSReader) readAvroRecord() (spill.SpanRecord, error) {
	if r.avroReader == nil {
...
func (r *GCSReader) readJSONLRecord() (spill.SpanRecord, error) {
...
func (r *GCSReader) jsonToSpanRecord(jr JSONSpanRecord) (spill.SpanRecord, error) {
	span := spill.SpanRecord{
		Name:          jr.Name,
		Kind:          jr.Kind,
...
func (r *GCSReader) Close(ctx context.Context) error {
	if r.closed {
		return nil
	}
...
func (r *GCSReader) Count() int {
	return r.spanCount
}

// parseGCSURL parses a GCS URL into bucket and object path
func (r *GCSReader) parseGCSURL(url string) (bucket string, object string, err error) {
	if !strings.HasPrefix(url, "gs://") {
		return "", "", fmt.Errorf("not a GCS URL: %s", url)
	}
